...
function pvpMitigationModToInput(m: ModifierSource): {
 value: number; itemId: string; itemName: string; confidence: ConfidenceLevel; notes?: string; chefRexEligible?: boolean;
} {
 const itemId = m.id.replace(/^(bridge|pipeline)-/, "").split("-playerDMGReduction")[0];
 const itemName = m.sourceLabel.replace(/ \(playerDMGReduction\)$/, "");
 let value = m.value;
 let chefRexEligible = false;
 if (m.sourceType === "food") {
  chefRexEligible = true;
  const f = foodBuffRegistry.find((x) => x.id === itemId);
  if (f && f.statModifiers) {
   const sm = f.statModifiers.find((s) => s.stat === "playerDMGReduction");
   if (sm) {
    value = sm.value;
   }
  }
...
export function buildCalculationInputFromSelection(
 ...
 const pvpMitigationResult = computePvPMitigation(
  ...
  build.food?.chefRex?.enabled ? build.food.chefRex.bonusPercent : 0,
  mode,
 );
