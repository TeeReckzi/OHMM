...
def bindict_int_hash(n: int) -> int:
    """Hash function for integer bindict keys."""
...
def bindict_string_hash(s: str) -> int:
    """Hash function for string bindict keys."""
    data = s.encode("utf-8")
    length = len(data)
...
def bindict_tuple_hash(tup: tuple) -> int:
    """Hash function for tuple bindict keys."""
    if len(tup) == 0:
        return 0x3C70706E

    h = 0x3C6EF373
    multiplier = 0xF4243
    i_var_9 = len(tup) * 2 + 0x14256

    for elem in tup:
        elem_hash = bindict_hash(elem)
        h = ((elem_hash ^ h) * multiplier) & 0xFFFFFFFF
        multiplier = (multiplier + i_var_9) & 0xFFFFFFFF
        i_var_9 = (i_var_9 - 2) & 0xFFFFFFFF
...
def bindict_hash(obj: Any) -> int:
    """Dispatch to the bindict hash function for a supported key."""
...
def try_bindict_hash(obj: Any) -> Optional[int]:
    """Return a bindict hash when the recovered key type is supported."""
    try:
        return bindict_hash(obj)
    except Exception:
        return None
    ...
    def _parse_hash_region(self, data: bytes):
        ...
        if marker == 0x56:
            ...
            for item in items:
                ...
                computed_hash = try_bindict_hash(key_value)
                item["computed_hash"] = computed_hash
                item["hash_match"] = computed_hash == item["hash"] if computed_hash is not None else None
...
def _json_safe(value: Any):
    """Convert parser output into JSON-serializable data."""
...
def _iter_input_files(input_path: Path, recursive: bool):
    ...
    for path in sorted(input_path.glob(pattern)):
        if path.is_file() and path.suffix.lower() in {".pyc", ".bin", ".dat"}:
            yield path


def parse_path(input_path: str, recursive: bool = False, debug: bool = False) -> Dict[str, Any]:
    """Parse one file or a directory of candidate bindict files."""
    root = Path(input_path).resolve()
    parser = BindictParser(debug=debug)
    ...
    for file_path in _iter_input_files(root, recursive):
        try:
            data = file_path.read_bytes()
            parsed = parser.extract_from_pyc(data)
            results["files"].append({
                "path": str(file_path),
                "relativePath": os.path.relpath(file_path, root if root.is_dir() else root.parent),
                "sizeBytes": len(data),
                "isBindictPyc": parser.is_bindict_pyc(data),
    ...
    results["fileCount"] = len(results["files"])
    results["parsedCount"] = sum(1 for entry in results["files"] if entry.get("parsed"))
    return results


def main() -> int:
    cli = argparse.ArgumentParser(
        description="Extract Neox/Messiah bindict payloads from .pyc or raw binary dictionary files."
    )
    cli.add_argument("input", help="Input .pyc/.bin/.dat file or directory.")
    cli.add_argument("-r", "--recursive", action="store_true", help="Recurse through an input directory.")
    cli.add_argument("-o", "--output", help="Write JSON output to this path instead of stdout.")
    cli.add_argument("--debug", action="store_true", help="Print parser debug errors while decoding.")
    cli.add_argument("--pretty", action="store_true", help="Pretty-print JSON output.")
    args = cli.parse_args()

    result = parse_path(args.input, recursive=args.recursive, debug=args.debug)
    ...
    has_errors = any("error" in entry for entry in result["files"])
